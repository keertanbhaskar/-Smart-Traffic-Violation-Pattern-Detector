...
def app(df):
    ...
    with col_right:
        st.success(
            'It visualizes the relationship between: Driver Profile and Risk Category (Low, Medium, High) Darker colors indicate higher number of violations.l')
    st.divider()
    # Create two columns: left (chart), right (table)
    col_left, col_right = st.columns([2, 1])
    # -------- LEFT: GRAPH --------
    ...
    # -------- RIGHT: TABLE --------
    with col_right:
        st.success("🚦 Repeat violations are most common among middle-aged drivers.")
    st.divider()
