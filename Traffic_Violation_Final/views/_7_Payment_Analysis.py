...
# PROFESSIONAL VERSION (NO ICONS | FIXED KEYS | COLORED)
# =====================================================
...
def app(df):
    ...
    # ---------------- TITLE ----------------
    st.markdown(
        "<h2 style='text-align:center;'>Payment Analysis Dashboard</h2>"
        "<p style='text-align:center;color:gray;'>Traffic Violation Payment & Revenue Insights</p>",
    ...
    global_df = df[
        (df["Payment_Method"].isin(pay_global)) &
        (df["Location"].isin(loc_global)) &
        (df["Time_of_Day"].isin(time_global))
    ]
    ...
    pay_filter = st.multiselect(
        ...
        default=global_df["Payment_Method"].unique(),
        key="pay_graphs"
    )
    ...
    with col2:
        ...
        g2_df = global_df[global_df["Payment_Method"].isin(pay_filter)]
    ...
    with col1:
        ...
        risk_df = global_df[global_df["Payment_Method"].isin(pay_filter_risk)]
        avg_risk = risk_df.groupby("Payment_Method")["Risk_Score"].mean().sort_values()
    ...
    with col2:
        ...
        fine_df = global_df[global_df["Payment_Method"].isin(pay_filter_fine)]
