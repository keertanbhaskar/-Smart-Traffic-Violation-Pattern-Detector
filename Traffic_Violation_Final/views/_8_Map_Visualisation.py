...
def app(df):
    ...
    def load_demo_data():
        ...
        return pd.DataFrame({
            ...
            'Violation_Type': np.random.choice(['Speeding', 'Drunk Driving', 'No Helmet'], 4000),
    ...
    # ==================== SIDEBAR (CLEAN: Page selector + CSV uploader) ====================
    with st.sidebar:
        st.markdown("<h4 style='color: #A08692;'>Map Visualisation</h4>", unsafe_allow_html=True)
        st.markdown("##### Upload CSV (optional)")
        uploaded_file = st.file_uploader("Drag and drop file here", type=["csv"], key="map_main_csv")

    # ==================== MAIN CONTENT ====================
    st.header("Map Analytics Dashboard")

    # CSV upload and processing (moved to sidebar)
    if uploaded_file is not None:
        try:
            df_uploaded = pd.read_csv(uploaded_file)
            # basic parsing
            if 'Date' in df_uploaded.columns or 'DateTime' in df_uploaded.columns:
                date_col = 'Date' if 'Date' in df_uploaded.columns else 'DateTime'
                df_uploaded[date_col] = pd.to_datetime(df_uploaded[date_col], errors='coerce')

            # attempt lightweight geocoding merge using STATE_COORDS
            coords_df = pd.DataFrame([{'Location': k, 'lat': v[0], 'lon': v[1]} for k, v in STATE_COORDS.items()])
            if 'Location' in df_uploaded.columns:
                df_uploaded = df_uploaded.merge(coords_df, on='Location', how='left')
            if 'lat' in df_uploaded.columns:
                df_uploaded['lat'] = df_uploaded['lat'].fillna(20.59)
            else:
                df_uploaded['lat'] = 20.59
            if 'lon' in df_uploaded.columns:
                df_uploaded['lon'] = df_uploaded['lon'].fillna(78.96)
            else:
                df_uploaded['lon'] = 78.96
            st.session_state.df = df_uploaded
            st.success(f"Loaded {len(df_uploaded):,} rows and applied lightweight geocoding.")
    ...
    if 'RoadCondition' not in df_base.columns:
        df_base['RoadCondition'] = np.random.choice(
            ["Potholes", "Wet", "Dry", "Under Construction"],
            len(df_base)
        )
    ...
    # ==================== NAVIGATION CARDS ====================
    st.markdown("###  **Filter by Section (Click to Navigate)**")
    ...
    nav_cards = [
        ...
        ("bi-currency-rupee", "Fine Severity Distribution")
    ]
    ...
    for i, (icon_class, title) in enumerate(nav_cards):
        with cols[i]:
            st.markdown(f'''
                <div style="text-align:center;padding:10px 8px;border-radius:12px;background:linear-gradient(135deg, rgba(15,13,21,0.88) 0%, rgba(42,37,51,0.88) 100%);border:1px solid #6C5C7C;box-shadow:0 8px 22px rgba(128,122,129,0.18);margin:2px;">
                    <i class="bi {icon_class}" style="font-size:24px;color:#E8DED9;"></i>
                    <div style="font-size:11px;margin-top:8px;color:#E8DED9;font-weight:600;">{title}</div>
                </div>
    ...
    with st.expander("Temporal Violation Patterns & Peak Hour Analysis", expanded=False):
        ...
        if "lat" not in df0.columns:
            if "lat_x" in df0.columns: df0["lat"] = df0["lat_x"]
            elif "lat_y" in df0.columns: df0["lat"] = df0["lat_y"]
        if "lon" not in df0.columns:
            if "lon_x" in df0.columns: df0["lon"] = df0["lon_x"]
            elif "lon_y" in df0.columns: df0["lon"] = df0["lon_y"]
        ...
        with c3:
            topn = st.slider("Top hotspots", 3, 12, 6, key="tod_topn_exp7")
        ...
        if india_geo is not None and not agg.empty:
            ...
            label_text = [f"<b>{loc}</b><br>{int(c):,}" for loc, c in zip(grouped_agg['Location'], grouped_agg['Cases'])]
