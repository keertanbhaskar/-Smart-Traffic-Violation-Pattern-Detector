...
def app(df):
    ...
    with col1:
        with st.expander("Monthly Violation Trend", expanded=True):
            ...
            ax.fill_between(range(len(months)), monthly.values, alpha=0.4, color="#cb4679")
    ...
    with col2:
        with st.expander("Yearly Violation Trend", expanded=True):
            ...
            ax.set_title("Year-wise Traffic Violations", color="#E8DED9", fontsize=13, fontweight='bold')
            ax.grid(True, linestyle="--", alpha=0.5, color="#4B5563")
            ax.set_facecolor('#2A2533')
            fig.patch.set_facecolor('none')
    ...
    with col2:
        with st.expander("Weekday vs Weekend", expanded=True):
            ...
            # Filter dataframe
            filtered_df = df_hour[df_hour["Violation_Type"].isin(selected_violations)]
    ...
    if year_filter:
        ...
        with col1:
            ...
            ax.fill_between(fine_trend.index, fine_trend["Fine_Amount"], alpha=0.3, color="#cb4679")

            ax.set_title("Average Fine Amount (Filtered Years)", color="#E8DED9", fontsize=14, fontweight='bold')
        ...
        st.info("Apply the year filter to view the fine trend.")
    ...
    st.success("This dashboard helps traffic authorities make informed decisions to improve road safety.")
